...
严格只输出 JSON,结构:
{
  ...
  "replies": [
    {"tone": "走心版", "text": "可直接发出去的回复"},
    {"tone": "直球版", "text": "可直接发出去的回复"}
  ],
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let userContent;
    if (mode === "translate") {
      const other = input.otherType === "F" ? "对方偏 F(在意感受)的表达,请按这个前提翻。"
                  : input.otherType === "T" ? "对方偏 T(直白就事论事)的表达,请按这个前提翻。"
                  : "对方偏 T 还是 F 不确定,你自己判断。";
      const self = input.selfType === "T" ? "用户自己偏 T,翻的时候多帮 ta 接住情绪层面。"
                 : input.selfType === "F" ? "用户自己偏 F,翻的时候点明对方的直白其实是善意、不是否定。"
                 : "";
      userContent = `请翻译这句话:\n${input.message}` +
        (input.context ? `\n\n情境:${input.context}` : "") +
        `\n\n${other}${self ? "\n" + self : ""}`;
